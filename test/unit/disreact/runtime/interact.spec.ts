...
describe('DisReact.interact', () => {
  it.effect('when processing interaction', E.fn(function* () {
    ...
    yield* E.promise(async () => await expect(JSON.stringify(discordDOM.reply.mock.calls[0][0], null, 2)).toMatchFileSnapshot('./.snap/interact-doken.json'));
    yield* E.promise(async () => await expect(JSON.stringify(discordDOM.reply.mock.calls[0][1], null, 2)).toMatchFileSnapshot('./.snap/interact-message.json'));
