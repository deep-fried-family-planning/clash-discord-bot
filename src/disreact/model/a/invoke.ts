...
export const invokeIntrinsicTarget = (root: Element.Element, event: Event.T) => {
  return invokeTargetInner(root, event);
};

const invokeTargetInner = (node: Element.Element, event: Event.T, original: Element.Element = node): Element.Element => {
