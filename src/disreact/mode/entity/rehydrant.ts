...
export declare namespace Rehydrant {
  export type Registrant = FC.FC.FC | El.El.El;
...
export const source = (input: Rehydrant.SourceId, id?: string): Rehydrant.Source => {
  if (FC.isFC(input)) {
    const fn = El.fn(input, {});
    if (id) {
...
export const fromSource = (source: Rehydrant.Source, props: any, data: any): Rehydrant.Rehydrant => {
  const cloned = El.fn(source.elem.type, props);
  return {
    id   : source.id,
    props: props,
    elem : cloned,
...
export const fromHydrator = (source: Rehydrant.Source, hydrator: Rehydrant.Hydrator, data: any): Rehydrant.Rehydrant => {
  const cloned = El.fn(source.elem.type, hydrator.props);
  return {
    id   : source.id,
...
export const hydrator = (rehydrant: Rehydrant.Rehydrant): Rehydrant.Hydrator => {
  return {
    ...
    stacks: Record.map(rehydrant.poly, (p) => p.stack),
  };
};
