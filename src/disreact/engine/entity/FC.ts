...
export const renderLocally = (self: Known, props: any, hooks: any): E.Effect<any> => {
  switch (self._tag) {
    case SYNC: {
      return E.succeed(self(props, hooks));
    }
    case ASYNC: {
      return E.promise(() => self(props, hooks) as Promise<any>);
    }
    case EFFECT: {
      return self(props, hooks) as E.Effect<any>;
    }
  }
  const out = self(props, hooks);
...
export const renderZero = (self: Known): E.Effect<any> => {
  switch (self._tag) {
    case SYNC: {
      return E.sync(self);
    }
