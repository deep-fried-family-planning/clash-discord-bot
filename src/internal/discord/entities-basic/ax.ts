...
export const make = (ix: IxIn) => {
  const _id    = cxRouter.parse('custom_id' in ix.data ? ix.data.custom_id : '');
  const _data  = CxVR.makeAccessorFromVR(_id);
  ...
  if (isModalIx(ix.data) && ix.type === Discord.InteractionType.MODAL_SUBMIT) {
    const cx = p(
      ...
      Ar.flatMap((cs, row) => p((cs as ActionRow).components, Ar.map((c, col) => p(
        ...
        Cx.map((cx) => p(cx, Cx.set('_data', CxVR.makeAccessorFromVR(cx._id)))),
      )))),
