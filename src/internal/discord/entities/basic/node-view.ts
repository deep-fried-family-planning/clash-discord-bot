import {Cv, Cx, Ev, Nx} from '#discord/entities/basic/index.ts';
...
export const render = (nv: T) => {
  console.info('view', nv.name);

  if (is('Dialog')(nv)) {
    const [dialog, ...components] = nv.func();

    return Nx.Dialog({
      name: nv.name,
      path: pipe(
        nv.path,
        Cx.Path.set('view', nv.name),
      ),
      title     : dialog.title,
      embeds    : [],
      components: pipe(
  ...
  const output     = nv.func();
  const embeds     = getEmbeds(output);
  const components = getComponents(output);
