...
import {type DA, Rf} from '#src/internal/disreact/model/entities/index.ts';
...
export const applyDefaultRefs = (ems: T[]) => {
  ...
  for (const em of ems) {
    if (!em.ref || em.ref.id === NONE) {
      (em as mut<typeof em>).ref = Rf.Default({id: `${default_ref_counter++}`});
    }
  }
  return ems;
};
...
const encode = (em: T) => {
  if (isMain(em)) {
    (em.data).image = {url: pipe(em.route, MainRoute.setRef(Rf.encode(em.ref!)), MainRoute.encodeUrl)};
    return em.data;
  }

  (em.data).image = {url: pipe(em.route, EmbedRoute.setRef(Rf.encode(em.ref!)), EmbedRoute.encodeUrl)};
  ...
  if (isDialogLinked(em)) {
    ...
    (em.data).image = {url: EmbedRoute.encodeUrl(route)};
  }
  return em.data;
};
