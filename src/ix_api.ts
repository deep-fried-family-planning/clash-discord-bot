...
const txmap = {
  [TxType.DEFERRED_CHANNEL_MESSAGE_WITH_SOURCE]: (body: IxD) => g(function * () {

  }),
  [TxType.DEFERRED_UPDATE_MESSAGE]: (body: IxD) => g(function * () {

  }),
  [TxType.MODAL]: (body: IxD) => g(function * () {
...
const component = (body: IxD) => E.gen(function * () {
  ...
  if (data.custom_id.startsWith('/v2')) {
    const id = cxRouter.parse(data.custom_id);

    const txtype = parseInt(id.defer ?? `${TxType.DEFERRED_UPDATE_MESSAGE}`);

    if (txtype === TxType.MODAL) {
    ...
    if (txtype === TxType.DEFERRED_CHANNEL_MESSAGE_WITH_SOURCE) {
      return r200({
        type: TxType.DEFERRED_CHANNEL_MESSAGE_WITH_SOURCE,
        data: {
    ...
    if (txtype === TxType.DEFERRED_UPDATE_MESSAGE) {
      return r200({
