...
const armTiming = (duration: Duration.Duration) => <R, E>(effect: E.Effect<void, E, R>) =>
  DokenState.use((ds) =>
    pipe(
      E.logTrace('dispatched'),
      E.andThen(effect),
      E.delay(duration),
      FiberHandle.run<any, any>(ds.timing, {
        onlyIfMissing        : false,
...
const armImmediate = <R, E>(effect: E.Effect<void, E, R>) =>
  DokenState.use((ds) =>
    pipe(
